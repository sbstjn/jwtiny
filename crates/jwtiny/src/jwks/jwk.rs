...
use crate::limits::{MAX_JWK_ALG_SIZE, MAX_JWK_E_SIZE, MAX_JWK_KID_SIZE, MAX_JWK_N_SIZE};
...
impl Jwk {
    ...
    pub(crate) fn to_key(
        ...
        // Convert based on algorithm (only RSA supported)
        match algorithm {
    ...
    fn validate_jwk_structure(&self, algorithm: &AlgorithmType) -> Result<()> {
        // Only RSA is supported for JWK conversion
    ...
    fn to_rsa_key(&self) -> Result<Vec<u8>> {
        ...
        // Decode Base64URL-encoded values with decoded size limits
